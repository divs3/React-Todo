...
const TodoItem = ({ item, id, onDelete, consumeEditedValue }) => {
  ...
  return (
    <div className="delete-list">
      {editTodoItem ? (
        <input onChange={(e) => setEditedText(e.target.value)} value={editedText} onKeyPress={editClickHandler} />
      ) : (
        <div>{item}</div>
      )}

      <button className="btn edit-list-item" onClick={() => setEditTodoItem((prevState) => !prevState)}>
        EDIT
      </button>
